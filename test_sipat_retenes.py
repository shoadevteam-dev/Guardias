"""Script para verificar que la lógica de retenes SIPAT funcione correctamente"""
...
with app.app_context():
    ...
    print(f"Fechas guardia SIPAT: {sipat_guardia_fechas}")
    
    print('\n=== MARZO 2026 - GUARDIAS Y RETENES ===')
    print('Fecha        Dia          Guardia              Retén                Es SIPAT?  Violacion?')
    print('-' * 100)
    ...
    for g in guardias:
        ...
        if disponibles:
            ...
            violacion = ''
            # Verificar si el retén tiene guardia mañana
            if persona_reten.id in sipat_guardia_fechas and dia_siguiente in sipat_guardia_fechas.get(persona_reten.id, set()):
                violacion = '*** VIOLACION: Reten SIPAT con guardia mañana ***'
                violaciones.append((g.fecha, persona_reten.nombre, 'retén hoy, guardia mañana'))
            
            dias = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo']
            marca_sipat = ' [SIPAT]' if es_reten_sipat else ''
            print(f'{str(g.fecha):<12} {dias[g.fecha.weekday()]:<10} {persona.nombre:<20} {reten_nombre:<20} {marca_sipat:<10} {violacion}')
    ...
    for i in range(len(sipat_retenes_ordenados) - 1):
        ...
        if diff == 1:
            violaciones_consecutivas.append((sipat_retenes_ordenados[i], sipat_retenes_ordenados[i+1]))
    
    print('\n' + '=' * 100)
    if violaciones:
        print(f'⚠ VIOLACIONES RETÉN-GUARDIA: {len(violaciones)} casos')
        for v in violaciones:
            print(f'  {v[0]}: {v[1]} ({v[2]})')
    else:
        print('✓ NO HAY VIOLACIONES: Ningún SIPAT es retén el día antes de su guardia')
    
    if violaciones_consecutivas:
        print(f'⚠ RETENES SIPAT CONSECUTIVOS: {len(violaciones_consecutivas)} casos')
        for v in violaciones_consecutivas:
            print(f'  {v[0]} -> {v[1]}')
    else:
