"""Script para debuggear el problema de las fechas de retenes"""
...
with app.app_context():
    ...
    print('\n=== RETENES MARZO 2026 (API LOGIC) ===')
    print(f'{"Fecha":<12} {"Dia":<10} {"Guardia":<20} {"Retén":<20} {"dia_anterior":<12} {"dia_siguiente":<12}')
    print('-' * 100)
    ...
    for g in guardias:
        ...
        if disponibles:
            ...
            for p in disponibles:
                ...
                print(f'{g.fecha.strftime("%d/%m/%Y"):<12} {g.fecha.strftime("%A"):<10} ', end='')
            ...
            print(f'{persona.nombre:<20} {persona_reten.nombre:<20} {dia_anterior.strftime("%d/%m"):<12} {dia_siguiente.strftime("%d/%m"):<12}')
            
            # Debug: mostrar quiénes fueron retén en días adyacentes
            if dia_anterior in reten_fechas_dict:
                for pid, fecha in reten_fechas_dict.keys():
                    if fecha == dia_anterior and pid == persona_reten.id:
                        print(f'  *** {persona_reten.nombre} YA FUE RETÉN EL {dia_anterior.strftime("%d/%m/%Y")} ***')
        else:
            persona = Persona.query.get(g.persona_id)
            print(f'{g.fecha.strftime("%d/%m/%Y"):<12} {g.fecha.strftime("%A"):<10} {persona.nombre:<20} {"SIN RETÉN":<20} {dia_anterior.strftime("%d/%m"):<12} {dia_siguiente.strftime("%d/%m"):<12}')
    ...
    for p in personas_activas:
        ...
        if count > 0:
            fechas_str = ', '.join([f.strftime('%d/%m') for f in sorted(retenes_por_persona.get(p.id, []))])
            print(f'{p.nombre}: {count} retenes -> {fechas_str}')
